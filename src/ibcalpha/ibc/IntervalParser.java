...
class IntervalParser {
    public static final DateFormat dateFormat = new SimpleDateFormat("HH:mm");

    private IntervalParser() {
    }

    public static List<Date> parse(String timesSetting)  throws IbcException {
        String[] times = timesSetting.split("[ ]+");
        if (! times[0].equalsIgnoreCase("Every")) {
            return convertSuppliedTimes(times);
        }

        return generateSaveTimes(times);
    }
